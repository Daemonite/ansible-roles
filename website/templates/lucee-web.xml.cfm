<?xml version="1.0" encoding="UTF-8"?><lucee-configuration {% if cfml_website_salt != '' %}hspw="{{cfml_website_password_hash}}" salt="{{cfml_website_salt}}"{% else %}pw="{{cfml_website_password_hash}}"{% endif %} version="4.2"><cfabort/>
...
<!--    or ask your database distributor                                                                -->
    <data-sources>
        ...
        {% elif cfml_datasource_type == 'mssql2005' %}
    ...
    <remote-clients directory="{lucee-web}remote-client/" log="logs/" log-level="info"/>
    ...
    define path to scedule task directory
        directory: path
    --> 
    <scheduler directory="{lucee-web}/scheduler/" log="{lucee-web}/logs/scheduler.log"/>
    ...
    <logging>
        ...
        <logger appender="resource" appender-arguments="path:{lucee-web}/logs/mail.log" layout="classic" name="mail"/>
        <logger appender="resource" appender-arguments="path:{lucee-web}/logs/scheduler.log" layout="classic" name="scheduler"/>
        <logger appender="resource" appender-arguments="path:{lucee-web}/logs/application.log" layout="classic" level="error" name="application"/>
        <logger appender="resource" appender-arguments="path:{lucee-web}/logs/exception.log" layout="classic" level="error" name="exception"/>
        <logger appender="resource" appender-arguments="path:{lucee-web}/logs/trace.log" layout="classic" level="info" name="trace"/>
    </logging>
